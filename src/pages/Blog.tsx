...
const Blog = () => {
  ...
  return (
    <>
      ...
      <div className="py-20 bg-gradient-to-b from-primary/10 to-transparent">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-20">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {posts.map((post) => (
            <article
              key={post.id}
              className="bg-white rounded-2xl overflow-hidden border border-gray-100 shadow-sm transition-all duration-300 hover:shadow-xl hover:scale-105 hover:border-primary/30"
            >
              <Link to={`/blog/${post.slug}`} className="block">
                <div className="relative h-52 overflow-hidden">
                  ...
                  <div className="absolute top-4 right-4">
                    <span className="bg-primary/90 backdrop-blur-sm px-4 py-1.5 rounded-full text-sm font-medium text-secondary">
                      {post.category}
                    </span>
                  </div>
                </div>
                <div className="p-6">
                  ...
                  <h2 className="text-xl font-semibold mb-3 text-gray-900 line-clamp-2 hover:text-secondary transition-colors">
                    {post.title}
                  </h2>
                  <p className="text-gray-600 mb-4 line-clamp-2">
                    {post.excerpt}
                  </p>
                  <span className="inline-flex items-center text-secondary font-medium hover:text-secondary/70 transition-colors">
          ...
          <div className="max-w-2xl mx-auto">
            ...
            <form className="flex gap-4 max-w-md mx-auto">
              <input
                ...
                className="flex-1 px-4 py-2 rounded-lg border border-white/20 bg-white/10 text-white placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary"
              />
              <button
                type="submit"
                className="px-6 py-2 bg-primary text-secondary font-semibold rounded-lg hover:bg-primary/90 transition-colors"
              >
                Subscribe
              </button>
            </form>
